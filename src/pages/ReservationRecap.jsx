...
import {
  RESERVATION_TICKETS,
  RESERVATION_PRICE_BY_TICKET,
} from "../components/reservationTickets";
...
const ReservationRecap = () => {
  ...
  const hasGoodiesAccess =
    String(draft?.mazeCode || "")
      .trim()
      .toUpperCase() === MAZE_GOODIES_CODE && Boolean(draft?.mazeCodeValidated);

  const cartItems = useMemo(() => {
    if (!draft?.quantities) return [];

    return RESERVATION_TICKETS.filter(
      (ticket) => Number(draft.quantities[ticket.id] || 0) > 0,
    ).map((ticket) => ({
      ...ticket,
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const normalizedEmail = String(draft.email || "")
        .trim()
        .toLowerCase();
      ...
      <div className="reservation-wrapper reservation-wrapper--narrow">
        {!isSuccess ? (
          <section
            ...
            aria-label={t("reservation_recap.aria.recap_section")}
          >
            <h1 className="reservation-main-title">
              {t("reservation_recap.title")}
            </h1>

            <div className="recap-container">
              <div className="recap-section">
                <h2 className="recap-title">
                  {t("reservation_recap.visit.section_title")}
                </h2>
                <div className="recap-row">
                  <span className="recap-label">
                    {t("reservation_recap.visit.date_label")}
                  </span>
                  <strong className="recap-value">{readableDate}</strong>
                </div>
                <div className="recap-row">
                  <span className="recap-label">
                    {t("reservation_recap.visit.slot_label")}
                  </span>
              ...
              <div className="recap-section">
                <h2 className="recap-title">
                  {t("reservation_recap.tickets.section_title")}
                </h2>
                {cartItems.map((item) => (
                ...
                {hasGoodiesAccess && (
                  <div className="recap-row">
                    <span className="recap-label">
                      {t("reservation_recap.tickets.goodies")}
                    </span>
                    <strong className="recap-value recap-value--valid">
                      {t("reservation_recap.tickets.goodies_price")}
                    </strong>
                  </div>
                )}
                <div className="recap-row total">
                  <span className="recap-label">
                    {t("reservation_recap.tickets.total")}
                  </span>
              ...
              <div className="recap-section">
                <h2 className="recap-title">
                  {t("reservation_recap.contact.section_title")}
                </h2>
                <div className="recap-row">
                  <span className="recap-label">
                    {t("reservation_recap.contact.last_name")}
                  </span>
                  <strong className="recap-value">{draft.lastName}</strong>
                </div>
                <div className="recap-row">
                  <span className="recap-label">
                    {t("reservation_recap.contact.first_name")}
                  </span>
                  <strong className="recap-value">{draft.firstName}</strong>
                </div>
                <div className="recap-row">
                  <span className="recap-label">
                    {t("reservation_recap.contact.email")}
                  </span>
                  <strong className="recap-value recap-email">
                    {draft.email}
                  </strong>
                </div>
              </div>
              ...
              <div className="recap-section">
                <h2 className="recap-title">
                  {t("reservation_recap.maze.section_title")}
                </h2>
                <div className="recap-row">
                  <span className="recap-label">
                    {t("reservation_recap.maze.code_label")}
                  </span>
                  <strong
          ...
          <section
            ...
            aria-label={t("reservation_recap.aria.success_section")}
          >
            <h2>{t("reservation_recap.success.title")}</h2>
            <p
              style={{ fontSize: "1.1rem", color: "#d4c4a0", margin: "1rem 0" }}
            >
